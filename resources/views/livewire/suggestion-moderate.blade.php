<div class="border-2 rounded-lg mb-6">
    @if (! isset($suggestions) || ! $suggestions->isNotEmpty())
        <div class="mx-auto w-70 my-10">
            ...
            <div class="text-gray-400 text-center font-bold mt-6">No suggestions found</div>
        </div>
    @else
        <div class="filters flex flex-col md:flex-row space-y-3 md:space-y-0 md:space-x-6 px-6 pt-6">
            <div class="w-36">
                <select wire:model="filter"
                    class="w-full rounded-xl border-blue font-semibold px-4 py-2">
            ...
            <div
                ...
                class="relative"
                x-init="
                    window.livewire.on('suggestionStatusWasUpdated', () => {
                        isOpen = false
                    })
                "
            >
                    ...
                    <form wire:submit.prevent="setStatus" action="#" class="space-y-4 px-4 py-6">
                        <div class="space-y-2">
                            <div>
                                <label class="inline-flex items-center">
                                    <input wire:model="status" type="radio" checked="checked"
                                        class="bg-gray-200 text-gray-600 border-none" name="radio-direct" value="awaiting">
                            ...
                            <div>
                                <label class="inline-flex items-center">
                                    <input wire:model="status" type="radio" name="radio-direct"
                                        class="bg-gray-200 text-blue border-none" value="considering">
                            ...
                            <div>
                                <label class="inline-flex items-center">
                                    <input wire:model="status" type="radio" name="radio-direct"
                                        class="bg-gray-200 text-purple border-none" value="planned">
                            ...
                            <div>
                                <label class="inline-flex items-center">
                                    <input wire:model="status" type="radio" name="radio-direct"
                                        class="bg-gray-200 text-yellow border-none" value="not_planned">
                            ...
                            <div>
                                <label class="inline-flex items-center">
                                    <input wire:model="status" type="radio" name="radio-direct"
                                        class="bg-gray-200 text-green border-none" value="done">
        ...
        </div>　<!-- end filters and set status -->
                ...
                <table class="divide-y divide-gray-200 table-fixed">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th scope="col"
                                class="px-6 py-3 text-left text-xs text-gray-700 uppercase tracking-wider w-2/3">
                                Title / Description
                            </th>
                            <th scope="col"
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($suggestions as $suggestion)
                            <tr>
                                <td class="px-4 py-4 space-y-1 border-l-2 {{ $suggestion->getTableBorderClasses() }}">
                                    <input
                                        wire:model="selectedSuggestions"
                                        type="checkbox"
                                        value="{{ $suggestion->id }}"
                                        class="rounded-full">
                                </td>
                                <td class="px-6 py-4 space-y-1">
                                    <a href="{{ route('suggestion.show', [$board, $suggestion]) }}" class="underline text-sm text-gray-900" target="_blank">
                                        {{ $suggestion->title }}
                                    </a>
                                ...
                                <td class="px-6 py-4 space-y-1 align-top">
                                    <div class="text-sm text-gray-900">{{ $suggestion->contributor->name }}</div>
                                    <p class="text-sm text-gray-500">{{ $suggestion->contributor->email }}</p>
                                </td>
                                <td class="px-6 py-4 space-y-1 align-top">
                                    <div class="text-sm text-gray-900">{{ $suggestion->created_at->diffForHumans() }}</div>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
